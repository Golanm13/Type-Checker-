...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isLitExp, LitExp ,
         CExp} from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeEmptyTupleTExp, ListTExp , makeListTExp , makeLiteralTExp } from "./TExp";
...
import { parseL5Program } from "./L5-ast";

// PairTExp type and helpers
export interface PairTExp {
    tag: "PairTExp";
    left: TExp;
    right: TExp;
}
export const makePairTExp = (left: TExp, right: TExp): PairTExp => ({ tag: "PairTExp", left, right });
export const isPairTExp = (te: TExp): te is PairTExp => (te as PairTExp).tag === "PairTExp";
...
//implmentation of L5typeof
export const L5programTypeof = (exp: string): Result<string> =>
    bind(p(exp), (sexp) =>
        bind(parseL5Program(sexp), (parsed) =>
            bind(typeofProgram(parsed, makeEmptyTEnv()), unparseTExp)));
...
export const typeofLit = (lit: LitExp, tenv: TEnv): Result<TExp> => {  // Compute the type of a literal expression
    const typeofSExpValueRecursive = (sexp: SExpValue, isTopLevel: boolean): Result<TExp> =>
        typeof sexp === 'number' ? makeOk(isTopLevel ? makeLiteralTExp() : makeNumTExp()) :
        typeof sexp === 'boolean' ? makeOk(isTopLevel ? makeLiteralTExp() : makeBoolTExp()) :
        typeof sexp === 'string' ? makeOk(isTopLevel ? makeLiteralTExp() : makeStrTExp()) :
        isSymbolSExp(sexp) ? makeOk(makeLiteralTExp()) :
        isEmptySExp(sexp) ? makeOk(makeEmptyTupleTExp()) :
        isCompoundSExp(sexp) ? bind(typeofSExpValueRecursive(sexp.val1, false), (firstTe: TExp) =>
                                  mapv(typeofSExpValueRecursive(sexp.val2, false), (secondTe: TExp) =>
                                      makePairTExp(firstTe, secondTe))) :
        makeFailure(`Unsupported S-expression value type in quoted expression: ${format(sexp)}`);

    return typeofSExpValueRecursive(lit.val, true);
}
        ...
        if (isPrimOp(app.rator)) {
            ...
            if (op === "car") {
                return bind(typeofExp(app.rands[0], tenv), (argT: TExp) =>
                    isPairTExp(argT) ? makeOk(argT.left) :
                    makeFailure(`car expected a pair, got ${unparseTExp(argT)}`));
            }
            if (op === "cdr") {
                return bind(typeofExp(app.rands[0], tenv), (argT: TExp) =>
                    isPairTExp(argT) ? makeOk(argT.right) :
                    makeFailure(`cdr expected a pair, got ${unparseTExp(argT)}`));
            }
            if (op === "list") {
                // List is a special case - it returns a tuple type of all the arguments
                return bind(mapResult((rand) => typeofExp(rand, tenv), app.rands),
                            (types: TExp[]) => 
                                types.length === 0 ? makeOk(makeEmptyTupleTExp()) :
                                allT((t: TExp): t is TExp => equals(t, types[0]), types) ? makeOk(makeListTExp(types[0])) :
                                makeFailure("All elements in list must have the same type"));
            }
        }
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> =>
    bind(typeofExp(exp.val, makeExtendTEnv([exp.var.var], [exp.var.texp], tenv)),
        (valTE: TExp) => bind(checkEqualType(valTE, exp.var.texp, exp),
...
export const typeofProgram = (prog: Program, tenv: TEnv): Result<TExp> => {
    ...
    const loop = (exps: List<Exp>, tenv: TEnv): Result<TExp> =>
        !isNonEmptyList(exps) ? makeFailure("No expressions in program") :
        isEmpty(rest(exps)) ? typeofExp(first(exps) as Exp, tenv) :
        isDefineExp(first(exps) as Exp) ?
            bind(typeofExp(first(exps) as Exp, tenv), _ =>
                loop(rest(exps) as List<Exp>, makeExtendTEnv([(first(exps) as DefineExp).var.var], [(first(exps) as DefineExp).var.texp], tenv))) :
            bind(typeofExp(first(exps) as Exp, tenv), _ => loop(rest(exps) as List<Exp>, tenv));

    return loop(exps, tenv); 
};
function mapResult(arg0: (rand: any) => Result<TExp>, rands: CExp[]): Result<TExp[]> {
