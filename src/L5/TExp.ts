...
export type TExp =  AtomicTExp | CompoundTExp | TVar | PairTExp | ListTExp | LiteralTExp;
...
export type ListTExp = { tag: "ListTExp"; elementTExp: TExp };
export const makeListTExp = (elementTExp: TExp): ListTExp => ({ tag: "ListTExp", elementTExp });
...
// uniform access to atomic types
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    (texp === "literal") ? makeOk(makeLiteralTExp()) :
    isString(texp) ? makeOk(makeTVar(texp)) :
    (isArray(texp) && texp[0] === "Pair" && texp.length === 3)
        ? bind(parseTExp(texp[1]), left =>
            bind(parseTExp(texp[2]), right =>
                makeOk(makePairTExp(left, right))))
    : (isArray(texp) && texp[0] === "List" && texp.length === 2)
        ? bind(parseTExp(texp[1]), (elementTExp) => makeOk(makeListTExp(elementTExp)))
    : isArray(texp)
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isListTExp(x) ? bind(unparseTExp(x.elementTExp), (e: string) =>
            makeOk(`(List ${e})`)) :
    ...
    makeFailure(`Unexpected TExp - ${JSON.stringify(x)}`);
